...
export const mapPetpoojaItem = (petpoojaItem, categoryNameById) => {
    ...
    return {
        ...
        image: normalizeImage(petpoojaItem?.item_image_url),
        categoryId,
...
export const getProcessedMenu = async ({ signal } = {}) => {
    ...
    const bestsellers = selectBestsellers(mappedItems, { max: 10, priceUnder: 150 });
    const bestsellerIds = new Set(bestsellers.map((b) => b.id));
    const itemsWithFlags = mappedItems.map((it) => ({
        ...it,
        isBestseller: bestsellerIds.has(it.id),
    }));
    ...
    return {
        ...
        grouped,
        bestsellers: itemsWithFlags.filter((i) => i.isBestseller),
    };
};
